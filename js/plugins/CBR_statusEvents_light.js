...
// Version 1.0.1
// 2016/8/18 1.0.1  status_lightと同じにする為TempからSystemに変更
// 2016/8/9 1.0.0

/*:
* @plugindesc エロステータス作れちゃうプラグイン(イベント用)
* @author COBRA
* @help Version 1.1
* 自由にいじれちゃうエロステータス画面(イベント用)
*
* ピクチャの位置は自由に配置できます
...
/*
change(
['txt_1',['sub','ほげええ'],['subColor','#FFFFF']]
['txt_2',['sub','ほげええ'],['subColor','#FFFFF']]
)
...
var parameters = PluginManager.parameters('CBR_statusEvents_light');

//ピクチャの取得
Game_System.prototype._CBR_eroStaEve_pic_list = [];
for(var i=1; i<=20; i++){
	var p =parameters['picName_'+i];
	if(p && p != 'CBR_OFF'){
		Game_System.prototype._CBR_eroStaEve_pic_list.push({
			'number':i,
			'name':p,
...
Game_System.prototype.CBR_eroStaEve_pic_list = function(){
	return this._CBR_eroStaEve_pic_list;
}


//テキストの取得
Game_System.prototype._CBR_eroStaEve_text_list = [];
for(var i=1; i<=30; i++){
	if(parameters['txtSubject_'+i] != 'CBR_OFF'){
		Game_System.prototype._CBR_eroStaEve_text_list.push({
			'number':i,
...
Game_System.prototype.CBR_eroStaEve_text_list = function(){
	return this._CBR_eroStaEve_text_list;
};
Game_System.prototype.CBR_eroStaEve_change = function(ary){
	...
	for(var i=0,len=ary.length; i<len; i++){
		var temp = ary[i][0].split('_');
		var type = temp[0].toLowerCase();
		var key = Number(temp[1]);

		for(var k=1,len2=ary[i].length; k<len2; k++){
			var sbj = ary[i][k][0];
			var data = ary[i][k][1];
			if(type == 'txt'){
				var ary3 = this.CBR_eroStaEve_text_list();
				for(var m=0,len3=ary3.length; m<len3; m++){
					if(ary3[m].number == key){
						sbj = txt_ary[sbj] || sbj;
						if(sbj=='fontSize' || sbj=='x' || sbj=='y' || sbj=='width' || sbj=='var'){data = Number(data);}
						this._CBR_eroStaEve_text_list[m][sbj] = data;
					}
				}
			}else if(type == 'pic'){
				var ary3 = this.CBR_eroStaEve_pic_list();
				for(var m=0,len3=ary3.length; m<len3; m++){
					if(ary3[m].number == key){
						if(sbj=='fontSize' || sbj=='x' || sbj=='y' || sbj=='width' || sbj=='var'){data = Number(data);}
						this._CBR_eroStaEve_pic_list[m][sbj] = data;
					}
				}
			}
		}
...
Game_Interpreter.prototype.pluginCommand = function(command){
	...
	if(command == 'CBR_statusEventL'){
		SceneManager.push(Scene_eroStaEvents);
	}
};
...
//########### Scene ###########
function Scene_eroStaEvents() {
	this.initialize.apply(this, arguments);
}
Scene_eroStaEvents.prototype = Object.create(Scene_MenuBase.prototype);
Scene_eroStaEvents.prototype.constructor = Scene_eroStaEvents;

Scene_eroStaEvents.prototype.initialize = function() {
	Scene_MenuBase.prototype.initialize.call(this);
};
Scene_eroStaEvents.prototype.create = function() {
	Scene_MenuBase.prototype.create.call(this);
	this._eroStaEventsWindow = new Window_eroStaEvents();
	this._eroStaEventsWindow.setHandler('cancel',   this.popScene.bind(this));
	this._eroStaEventsWindow.setHandler('ok',   this.popScene.bind(this));

	this._eroStaEventsWindow._margin = 0;//背景ピッチリする為
	this._eroStaEventsWindow.margin = 0;//本当はこっちだけどね
	this._eroStaEventsWindow._windowFrameSprite.visible = false;//枠線を消す

	this.addWindow(this._eroStaEventsWindow);
};

//########### window ###########
function Window_eroStaEvents() {
	this.initialize.apply(this, arguments);
}

Window_eroStaEvents.prototype = Object.create(Window_Selectable.prototype);
Window_eroStaEvents.prototype.constructor = Window_eroStaEvents;

Window_eroStaEvents.prototype.initialize = function() {
...
Window_eroStaEvents.prototype.CBR_complete = function(){//描写終わったかどうか
	return this._CBR_complete;
}

Window_eroStaEvents.prototype.update = function() {//毎回image描写してもいいけどそれだと負荷が多くなるのでこの方法で
	Window_Selectable.prototype.update.call(this);
	
	if(!this.CBR_complete()){//まだ画像の描写してないのなら
		var flag = false;
		var ary = $gameSystem.CBR_eroStaEve_pic_list();
		for(var i=0,len=ary.length; i<len; i++){
			var name = ary[i].name;
		...
		if(!flag){//全ロードが終わってたら
			...
			//テキストの表示
			this.resetTextColor();
			var ary = $gameSystem.CBR_eroStaEve_text_list();
			for(var i=0,len=ary.length; i<len; i++){
...
Window_eroStaEvents.prototype.drawPicture = function(dir, filename,sx, sy, x, y, w, h, zoom, hue) {
...
Window_eroStaEvents.prototype.standardPadding =function(){
	return 0;
};
